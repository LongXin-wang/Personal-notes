...
async def async_task():
    print("Start async task")
    await asyncio.sleep(5)  # 模拟耗时操作，比如网络请求
    print("Async task completed")
    return "Result from async task"

# 超时回调函数
def timeout_callback(future):
    if future.cancelled():
        print("Task was cancelled due to timeout")
    else:
        print("Task took too long, it timed out")

# 主函数
async def main():
    print("Start main function")
    try:
        result = await asyncio.wait_for(async_task(), timeout=3)  # 设置超时时间为3秒
        print("Task result:", result)
    except asyncio.TimeoutError:
        timeout_callback(asyncio.Task.current_task())
